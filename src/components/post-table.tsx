...
export function PostTable({
  posts, emptyText = "등록된 글이 없습니다.", showCourse = true,
}: { posts: PostListItem[]; emptyText?: string; showCourse?: boolean }) {
  return (
    <div className="rounded-lg border border-border bg-card overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-border">
          {posts.length === 0 && (
            <tr>
              <td colSpan={6} className="p-12 text-center text-muted-foreground">{emptyText}</td>
            </tr>
          )}
          {posts.map((p, i) => {
            ...
            return (
              <tr key={p.id} className="hover:bg-secondary/40 transition-colors">
                ...
                <td className="p-3 text-xs text-muted-foreground inline-flex items-center gap-1 mt-3">
